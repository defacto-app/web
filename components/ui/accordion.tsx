...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn("grid justify-items-start", className)}
    {...props}
  />
));
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex justify-between w-full">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 place-content-center justify-between py-4 font-medium transition-all hover:underline [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
